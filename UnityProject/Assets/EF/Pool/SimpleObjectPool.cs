...
namespace EF
{
    ...
    public interface ISimpleObjectPool<T> where T : class
    {
        ...
        /// <summary>
        /// 回收对象 如果对象在timer秒后没有被使用，则销毁对象
        /// </summary>
        /// <param name="obj"></param>
        /// <param name="timer"></param>
        void RelaseDestroy(T obj, float timer);
    ...
    public class SimpleObjectPool<T> : ISimpleObjectPool<T> where T : class
    {
        ...
        public void RelaseDestroy(T obj, float timer)
        {
            if (obj == null)
            {
                Log.Error("尝试释放空对象到对象池");
            ...
            if (timer > 0)
            {
                DelayDestroy(obj, timer).Forget();
            }
        }

        private async UniTaskVoid DelayDestroy(T obj, float timer)
        {
            await UniTask.Delay((int)(timer * 1000));
            if (_pool.Contains(obj))
            {
                // 从池中移除对象 Stack 不支持 Remove
                _pool.Pop();
            }
            _onDestroy?.Invoke(obj);
        }
