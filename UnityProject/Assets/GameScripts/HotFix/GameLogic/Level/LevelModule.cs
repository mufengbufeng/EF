...
namespace GameLogic
{
    public sealed class LevelModule : AEFManager, ILevelModule
    {
        ...
        public LevelModule(ISaveManager saveManager, EventHub eventHub)
        {
        ...
        public void RecordKill()
        {
            ...
            // 触发进度变化事件
            _eventHub.LevelProgressChangedEvent.Publish(
                new LevelProgressChangedEvent(_currentLevelId, _killCount, _requiredKills));

            // 检测通关
            if (CheckLevelComplete())
            {
                Log.Info($"[LevelModule] Level {_currentLevelId} complete!");
                _eventHub.LevelCompleteEvent.Publish(new LevelCompleteEvent());
            }
        }
