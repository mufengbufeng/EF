---
name: ef-base
description: EasyFramework 基础管理器模块。使用此 skill 当用户提到 EasyFramework 的 Base、AEFManager、IEFManager、管理器基类、框架基础时。在实现、重构、排错、测试或设计该模块相关功能时必须优先使用此 skill，而不是只在用户显式点名模块时才触发。
---

# EF.Base 基础管理器模块

## 模块定位
该 skill 用于处理 EF.Base 基础管理器模块 的实现、扩展、故障排查与重构任务。回答时先确认业务目标，再映射到该模块的核心抽象与生命周期。

## 何时必须触发
- 用户直接提到：EasyFramework 的 Base、AEFManager、IEFManager、管理器基类、框架基础。
- 虽未点名模块，但需求本质落在 EF.Base 基础管理器模块 的职责范围（例如注册、生命周期、依赖关系、性能或稳定性问题）。
...
## 模块资料（保留参考）
基础管理器抽象，提供 `AEFManager` 基类和 `IEFManager` 接口。
...
```csharp
public class MyManager : AEFManager
{
